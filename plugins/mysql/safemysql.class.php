...
class SafeMySQL implements iSqlJsonDb
{
...
//print $query;
	...
	protected function escapeFloat($value)
	{
		...
		if(!is_numeric($value))
		{
			$this->error("Integer (?f) placeholder expects numeric value, ".gettype($value)." given");
			return FALSE;
		}
		if (is_float($value))
		{
			$value = number_format($value, 7); // may lose precision on big numbers
		} 
		return $value;
	}
  ...
  global $pager;
    ...
    $count = $count_request ? $this->getOne($count_request) : -1;
    $page = $offset = 0;

    while ($useget){
      if (isset($_GET['limit']))
        $limit = 0 + $_GET['limit'];
      if ($count>=0 && isset($_GET['page'])){
        $page = 0 + $_GET['page'];
        $offset = $limit * $page;
        break;
      }
    ...
    $this->query("call _sql_to_json_proc('".$request."',@j);");
    $this->clear_result();
    $into = $this->getOne("select @j;");
    $into = json_decode($into,TRUE);
    $pager = array(
      '#page'=>$page,
      '#limit'=>$limit,
      '#offset'=>$offset,
  ...
  global $db;
    $node_ids = explode(',',$node_id);
    $node_id='';
    foreach($node_ids as $n) $node_id.=($node_id?',':'').$n;

    $par = '';
    if ($useget){
      if (isset($_GET['page'])){
        if (!is_array($_GET['page']))
            $params['default.page'] = 0+$_GET['page'];
        else {
          foreach($_GET['page'] as $i=>$v){
            if(preg_match("/^table_[a-zA-Z0-9_]*$/i",$i))
            $params[$i.'.page'] = 0+$v;
          }
        }
      }
      if (isset($_GET['limit'])){
        if (!is_array($_GET['limit']))
            $params['default.limit'] = 0+$_GET['limit'];
        else {
          foreach($_GET['limit'] as $i=>$v){
            if(preg_match("/^table_[a-zA-Z0-9_]*$/i",$i))
              $params[$i.'.limit'] = 0+$v;
          }
        }
      ...
      if (isset($_GET['offset'])){
        if (!is_array($_GET['offset']))
            $params['default.offset'] = 0+$_GET['offset'];
        else {
          foreach($_GET['offset'] as $i=>$v){
            if(preg_match("/^table_[a-zA-Z0-9_]*$/i",$i))
              $params[$i.'.offset'] = 0+$v;
          }
        }
    ...
    if (is_array($params)) foreach($params as $i=>$v){
      $par.= ($par?';':'').$i.'="'.$v.'"';
    }
    $par = str_replace("'","\'",$par);
    $this->query("call get_nodes('".$par."','".$node_id."',@j);");
    $this->clear_result();
    $t = $this->getOne("select @j;");
  
  
    return $t;
  }

  // Функция загрузки одной ноды по url адресу
  // $link - url адрес, или пустота если требуется страница а адресом index
  // $params - параметры - массив типа "table_test.row_limit" = >10
  // $useget - использовать параметр "page" из GET запроса
  public function node_load_by_link($link, $params=array(), $useget=TRUE){
  global $db;
    ...
    $par = str_replace("'","\'",$par);
    $this->query("call get_nodes_by_link('".$par."',?s,@j);",$link);
    $this->clear_result();
    $t = $this->getOne("select @j;");
  ...
  global $db;
    ...
    $par = str_replace("'","\'",$par);
    $this->query("call _sqlnode_to_json_proc('".$par."',?s,@j);",$request);
    $this->clear_result();
    $t = $this->getOne("select @j;");
  ...
  public function get_field($nid,$table,$column){
    if (!check_table_column($table,$column)) throw new Exception('Недопустимая таблица или колонка.');
    try{
      $field = $this->getOne('SELECT ?n FROM ?n WHERE nid = ?i',$column, $table, $nid);
    } catch (Exception $e) {
      throw new Exception('Недопустимая таблица или колонка.');
    }
    return $field;
  }
